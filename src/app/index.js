...
const weatherCopy = (code, isDay = 1) => {
  if (code === 0) return isDay ? ['Clear sky', '☀️'] : ['Clear night', '🌙'];
  if ([1, 2].includes(code)) return isDay ? ['Mostly sunny', '🌤️'] : ['Mostly clear night', '🌙'];
  if (code === 3) return isDay ? ['Cloudy', '☁️'] : ['Cloudy night', '☁️'];
...
const tagumBarangays = [
  'Apokon', 'Bincungan', 'Busaon', 'Canocotan', 'Cuambogan', 'La Filipina',
  'Liboganon', 'Madaum', 'Magdum', 'Mankilam', 'New Balamban', 'Nueva Fuerza',
  'Pagsabangan', 'Pandapan', 'Magugpo Poblacion', 'San Agustin', 'San Isidro',
  'San Miguel', 'Visayan Village', 'Magugpo East', 'Magugpo North',
  'Magugpo South', 'Magugpo West',
];

function tagumSearchTerm(query) {
  const normalized = query.toLowerCase().replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim();
  const barangay = tagumBarangays.find(name => {
    const normalizedName = name.toLowerCase();
    return normalized === normalizedName || normalized.includes(`${normalizedName} tagum`) ||
      normalized.startsWith(`barangay ${normalizedName}`);
  });
  return barangay ? `${barangay}, Tagum City, Davao del Norte, Philippines` : null;
}

async function findPhotonPlaces(term, count, scorePlace) {
  const response = await fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(term)}&limit=${count}&lang=en`);
  ...
  const places = (result.features || []).map(feature => {
    ...
    return {
      name: properties.name || properties.city || properties.locality || properties.district || properties.county,
      admin1: properties.state,
      admin2: properties.county,
      admin3: properties.city,
      admin4: properties.district || properties.locality,
...
async function findPlaces(query, count = 8) {
  ...
  const scorePlace = place => {
    const details = [place.name, place.admin1, place.admin2, place.admin3, place.admin4, place.country]
      .filter(Boolean).join(' ').toLowerCase();
    return contextWords.reduce((total, word) => total + (details.includes(word) ? 1 : 0), 0);
  };

  const localTagumTerm = tagumSearchTerm(trimmed);
  if (localTagumTerm) {
    try {
      const localPlaces = await findPhotonPlaces(localTagumTerm, count, scorePlace);
      if (localPlaces.length) return localPlaces;
    } catch {
      // Keep the main city geocoder available if the locality fallback is offline.
    }
  }
  ...
  for (const term of uniqueTerms) {
    ...
    if (result.results?.length) {
      return result.results.sort((a, b) => scorePlace(b) - scorePlace(a));
    }
  }

  // Open-Meteo focuses on cities and postal codes. Photon adds villages,
  // barangays, districts, and other OpenStreetMap localities as a fallback.
  for (const term of uniqueTerms) {
    if (term.length < 2) continue;
    let places;
    try {
      places = await findPhotonPlaces(term, count, scorePlace);
    } catch {
      continue;
    }
    if (places.length) return places.sort((a, b) => scorePlace(b) - scorePlace(a));
  }
  return [];
}
...
export default function WeatherScreen() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => search('Manila'), 0);
    return () => clearTimeout(timer);
    // Load the default city once when the screen mounts.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const current = weather?.data.current;
  const daily = weather?.data.daily;
  const [condition, emoji] = current ? weatherCopy(current.weather_code, current.is_day) : ['Your forecast awaits', '☀️'];
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.searchRow}>
          <View style={styles.searchBox}>
            ...
            <TextInput ref={inputRef} style={styles.searchInput} value={cityInput} onChangeText={updateCityInput} onSubmitEditing={() => search(cityInput)} placeholder="Search any city worldwide..." placeholderTextColor="#83aeb2" returnKeyType="search" autoCapitalize="words" accessibilityLabel="Search cities worldwide" />
            {cityInput.length > 0 && <Pressable onPress={() => updateCityInput('')} hitSlop={8} accessibilityLabel="Clear search"><Text style={styles.clearText}>×</Text></Pressable>}
          </View>
        ...
        {emptyInput && <View style={styles.inputValidation}><Text style={styles.inputValidationIcon}>ⓘ</Text><Text style={styles.inputValidationText}>Enter a city or municipality to search.</Text></View>}
...
const styles = StyleSheet.create({
  inputValidation: { flexDirection: 'row', alignItems: 'center', gap: 7, paddingHorizontal: 12, paddingVertical: 9, marginTop: -2, marginBottom: 10, borderRadius: 12, backgroundColor: 'rgba(255,244,223,0.85)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.7)' },
  inputValidationIcon: { color: '#bd8841', fontSize: 14 },
  inputValidationText: { color: '#926d3b', fontSize: 12, fontWeight: '700' },
